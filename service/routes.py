...
def list_products():
    ...
    if query_name:
        ...
        category = getattr(Category, query_category.upper())
        products = Product.find_by_category(category)
    elif query_available:
        available_value = query_available.lower() in ["true", "yes", "1"]
...
def get_products(product_id):
    ...
    # convert the product to a dictionary and send a 200 OK
...
def update_products(product_id):
    ...
    try:
        ...
        abort(status.HTTP_400_BAD_REQUEST,
              f"Product with id '{product_id}' was not updated: {error.args[0]}")
